...
function ListAllUserInTree()
{

	# LDAP Search from base of tree
	{
	ldapsearch -v -x -w $EDIR_AUTH_STRING  -D $EDIR_USER -h ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_ADDRESS_ONE} -p ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_PORT_ONE} '(&(objectClass=user)(cn=*))' | grep -vE "fullName:" | grep -e "dn:" | sed 's/.*dn: //'
	} 1> /dev/null 2> /dev/null
...
OutputMessage "Example: -Action \"GroupReport\""
OutputMessage "Example: -Action \"DisableUser\""
OutputMessage "Example: -Action \"ListUsers\""
